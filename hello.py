...
def assignAngle(keys,angles):
    ...
    if keys["Right"]==True:
        ...
        angle+=5*(math.pi/180)
    return angle
#def draw_obstacle(width):
    #for i in range(5):
        #pygame.draw.rect(sc,'green',(pos_x+width*i,420,30,30))
while run:
    ...
    #if pos_x<5:
    #    pos_x+=random.randint(400,500)
    #if(pos_x>500):
    #    pos_x=random.randint(0,10)
    clk.tick(80)
    sc.fill('orange')
    #pygame.draw.rect(sc,'red',(0,450,500,50))
    #pygame.draw.rect(sc,'yellow',(250,400,40,50))
    #draw_obstacle(width)
